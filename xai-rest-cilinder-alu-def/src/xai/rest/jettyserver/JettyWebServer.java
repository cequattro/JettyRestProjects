...
public class JettyWebServer extends Thread {
	...
	public void run() {
		...
		System.out.println("[ESBORRARRRR] startJettyWebServer");
		ServletContextHandler context = new ServletContextHandler(ServletContextHandler.SESSIONS);
		context.setContextPath("/");

		jettyServer = new Server(port);
		jettyServer.setHandler(context);
		System.out.println("[ESBORRARRRR] startJettyWebServer -- port: "+port);

		ServletHolder jerseyServlet = context.addServlet(
				org.glassfish.jersey.servlet.ServletContainer.class, "/*");
		jerseyServlet.setInitOrder(0);

		System.out.println("[ESBORRARRRR] startJettyWebServer -- 2: ");
		...
		try {
			jettyServer.start();
			System.out.println("[ESBORRARRRR] startJettyWebServer -- DESPRES start: ");
